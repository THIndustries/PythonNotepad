...
def main():
    ...
    while True:
        ...
        print("3. filter - Фильтрация заметок по дате")
...
'''
Основной блок if __name__ == "__main__": вызывает функцию main() для запуска приложения, 
если код выполняется как самостоятельный исполняемый файл (а не импортируется как модуль в другой код).
'''
